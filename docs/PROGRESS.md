...
<!-- HNS:STATE -->
- **단계**: 카드형 세로 리듬 **배포 완료 → 사용자 라이브 검증 대기**.
  `main`=`7d55060`(PR #311) · 회차 **미종결**(검증 전)
- **상태**: verifying
- **갱신**: 2026-08-24
- **배포 판정 근거** `[측정]`: `main.dart.js` last-modified 08-21 09:19 → **08-23 23:35** ·
  content-length 5,515,205 → **5,515,435** · deploy-nas verify-live success
- **승인값(불변)**: 항목 사이 **20.0dp @390 · 25.0 @960**. 이제 3계열이 같은 값을 지난다:
  우리 타일 `kBbSpaceSpec[xs]` · `ListTile` `listRowPadV`(10,12.5) ·
  **카드 `margin xs` + `cardRowPadV`(6, 8.5)**
- **사후 실측**: 카테고리별·결제수단별·요약탭 **20.0/25.0 정확 일치** ·
  전년비교 **28.5/32.5** · 주간카드 **41.0/45.5**(초과분은 카드 안쪽 잉크 — 아이콘 블록
  36dp · 진행바 5dp. 빈 공간 자체는 승인값과 같다) · 예산 월간 항목은 **불변**
- **오진 정정**: 사용자가 "예산 미적용"으로 본 대상은 항목(이미 20.0)이 아니라 **주간 카드**
- **가드**: 세로 리듬 V1/V1-b/**V3**(카드) · 채택 가드(호스트 9개 열거 · 맨 `Card(` 0건 ·
  `DashboardPage` 미도달) · 계측기 2종 상주 · 리터럴 ratchet **1464**
- **CI 게이트(6)**: analyze 신규 0 / `flutter test` **1105** / `./gradlew test` /
...
<!-- HNS:NEXT -->
**사용자 라이브 검증 대기.** 배포는 끝났고(`7d55060`, 산출물 신선도 확인) 남은 것은 판정뿐이다.

### 0단계 — 오프라인 배너 없음 확인

배너가 있으면 수정 전 수치가 그대로 보여 "배포 미반영"과 구분이 안 된다
(`feedback_live_verification_online_precheck`). 모바일은 **새로고침 1회** 권장.

### 확인할 화면과 기대값 (모바일 390 기준 · 텍스트↔텍스트)

1. 분석 > 통계 > **카테고리별** — 34.2 → **20.0dp**(거래 목록과 같은 리듬이어야 한다)
2. 분석 > 통계 > **결제수단별** — 32.0 → **20.0dp**
3. 분석 > 통계 > **전년 비교** — 62.7 → **28.5dp**(초과분은 카드 안 아이콘 블록 36dp)
4. 분석 > 예산 > **주간** 카드 — 75.2 → **41.0dp**(초과분은 카드 안 진행바)
5. 분석 > 예산 > **월간 항목** — **변화 없음**. 이미 승인값이었다.
   사용자가 "예산 미적용"으로 본 대상은 항목이 아니라 주간 카드였다 — 이 점을 함께 확인받는다
6. 카드 **테두리 사이는 전 표면 8.0dp 균일**

### 판정이 갈리면 다음 레버 (사전 선언)

- "여전히 넓다"(전년비교·주간카드) → **카드 안쪽**을 줄인다: 아이콘 블록 36 → 28dp ·
  진행바 앞 간격 4 → 0. 여백은 이미 승인값이라 더 줄이면 카드가 붕괴한다
- "카드가 답답하다" → 레버는 `cardRowPadV.min`(6.0) 하나. 올리는 만큼 margin 을 내려 상쇄
- "카드끼리 붙어 보인다" → `space.xs`(4.0)를 올린다(사이가 같이 늘어난다 — 승인값 재조정 필요)
...
6. **대장 롤오프** — 본체 134KB(150KB 부터 차단) → `progress-rolloff.py`
