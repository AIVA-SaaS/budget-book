...
<!-- HNS:STATE -->
- **단계**: **분석 탭 개편 회차 — 배포 완료. 남은 것은 사용자 라이브 검증뿐.**
  PR **#299** · `main` = **`6bc3c86`** · 번들 `last-modified` 2026-08-18 05:29:51 GMT
...
<!-- HNS:NEXT -->
**코드·배포는 끝났다. 남은 것은 사용자 라이브 검증뿐이다** — 통과하면 회차 종결.

시나리오 정본은 기획서 §9: `docs/sessions/2026-08-18_1_analysis-tab-density_plan.md`

- **0단계(필수 선행)**: 오프라인 배너 없음 확인. 있으면 재연결 후 시작 —
  오프라인이면 수정 전 수치가 그대로 보여 "배포 미반영"과 구분 불가
- **A 분석 탭 밀도**: [분석]→[통계] 에서 **월말 점검 카드가 안 보일 것**. 크롬 대비 데이터 비율 판정
- **B 아이콘·버튼 전수**: 하단 네비 / 거래 폼 [지출][수입][이체] 탭(아이콘 유지된 채 낮아짐) /
  월 이동 화살표가 여전히 누르기 쉬운지(44dp 하한 체감)
- **C 자산 카드 2줄**: 신용카드가 `이름` / `마감·결제`+칩 **2줄**로 끝나는지. 계좌는 종전과 동일
- **D 회귀**: 거래 탭 뷰 토글 3번째 세그먼트로 **정산 뷰 진입 가능**한지(월말 점검 카드를
  지운 뒤에도 기능이 사는지) / 분석 탭 월 이동이 두 sub-tab 에 동기되는지 / 통계 4탭·기간 필터

⚠ **미리 알린 설계 귀결**: 본문 타이포는 1440px 아래에서 **상수**다(가독성 — 의도).
모바일 개선은 큰 글자·아이콘·크롬·밀도에서 온다. **"글자가 여전히 크다"** 회신이 오면
레버는 `kBbTextSpec.min` 하향 **1곳**, **"작아 읽기 힘들다"** 면 현행 유지다(사전 판정 기준).

통과 시: 결과 문서 `docs/sessions/2026-08-18_1_..._result.md` + 회차 종결.
실패 시: 기획서 §8 미해결의 사전 판정 기준대로 레버만 움직인다(재기획 아님).
