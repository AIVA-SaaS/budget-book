...
<!-- HNS:STATE -->
- **단계**: 세로 리듬 3차 **부분 통과** → **카드형 회차 착수 대기**(2026-08-24).
  PR #308 배포분(`ListTile` 87곳 + 우리 타일)은 **사용자 판정 "잘 되고 마음에 든다"**.
  `main`=`8320c1a` · 회차 **미종결**(카드형 미적용)
- **확정된 승인값**: 항목 사이 **20.0dp @390 · 25.0 @960**.
  우리 타일 = `kBbSpaceSpec[xs]`(4,4) · `ListTile` = `listRowPadV`(10,12.5) +
  `listRowMinHeight`(34,41). **세 값이 같은 사이를 만들도록 가드 9종이 묶여 있다**
- **미적용 영역(다음 회차)**: **카드형 목록** — `Card(` **169건 / 52파일**.
  통계 항목 사이 **34.2dp @390 / 48.0 @960** `[측정 2026-08-24]`
- **첫 행동**: §3 NEXT 의 카드형 착수 실측(재조사 불필요)을 읽고 **기획부터**.
  카드는 "사이"의 정의가 둘(테두리↔테두리 vs 텍스트↔텍스트)이라 **정의부터 확정**한다
- **1·2차 실패 교훈(유지)**: 사용자 지표를 내 지표로 치환하지 마라 ·
  요청이 한 축이면 한 축만 · 렌더 기하로 재고 체감 %를 사다리로 제시
  → `feedback_user_metric_definition`
- **가드**: 스윕 37건 · 계단 4종 · 타일 8종 · **세로 리듬 9종** · 리터럴 ratchet 1481 ·
  일관성 래칫(A0/C0/D0/G0)
- **CI 게이트(6)**: analyze 신규 0 / `flutter test` **1074** / `./gradlew test` /
  `build web --release` / `check_ui_scaling.py` / `audit_ui_consistency.py`
- **blocker**: 없음(다음 세션에서 기획 착수) · **갱신**: 2026-08-24
<!-- /HNS:STATE -->
...
<!-- HNS:NEXT -->
**카드형 목록의 항목 사이 좁히기 — 기획부터. 승인 게이트는 기획 단계뿐.**

### 요청 (사용자, 2026-08-24 원문)

> 모두 적용해달라고 했는데 분석 내 예산/통계에는 적용되지 않은 것 같다. 이건 카드 쪽이라
> 적용 안된건가? 확인 후 맞으면 clear 후 다음 진행과 카드에도 줄이는 작업 포함 후 적용

### 착수 실측 — 재조사 불필요 `[측정 2026-08-24]`

- 목표값(이미 승인·라이브 검증 통과): **항목 사이 20.0dp @390 · 25.0dp @960**
- 카드형 현재값: **34.2dp @390 · 48.0dp @960**
- 분해(카드 한쪽): 바깥 `Padding(v: xs)` **4.0** + `cardTheme.margin` 세로 `sm` **5.1**
  + 카드 내부 `Padding(all: lg)` **8.0** = 17.1 → 사이 34.2 (@960: 4 + 8 + 12 = 24 → 48.0)
- 대표 구조: `category_breakdown_tab.dart:383` (`Padding → Card → InkWell → Padding`)
- 전수: `Card(` **169건 / 52파일**. `cardTheme.margin` 은 **전역**이라 한 줄로 169건에 닿는다
- ⚠ 죽은 화면 주의: 최다 보유 `dashboard_page.dart`(27건)는 **미라우팅**
  (`reference_dead_home_dashboard`) — 작업량 산정에서 제외하고 가드로 확인

### 기획에서 **먼저 확정할 것** — "카드의 사이"는 두 지표다

1. **테두리↔테두리**(빈 공간) = 2 × (바깥 Padding + `cardTheme.margin`) = **18.2dp @390**
   → 이미 20 근처다. 즉 사용자가 넓다고 본 것은 이 지표가 아니다
2. **텍스트↔텍스트** = 위 + 2 × 카드 내부 Padding = **34.2dp @390** ← 사용자 지표
   (1·2차에서 확인된 사용자 언어: "1과 2 사이")

목표 20 을 텍스트 기준으로 맞추려면 **카드 내부 padding 을 줄여야 한다** — 카드가 답답해질
수 있으므로 후보를 수치 사다리로 제시하고 사용자가 값을 고르게 한다(1차 실패 교훈).

**후보(390 기준 · 바깥 Padding 제거 전제)**
- ⓐ margin `xs`(4) + 내부 `lg`(8) → 사이 **24.0** (@960 32.0) — 카드 여백 보존
- ⓑ margin `xs` + 내부 `md`(6.4) → 사이 **20.8** (@960 28.0) — 목표 근사
- ⓒ margin 0 + 내부 `lg` → 사이 **16.0** (@960 24.0) — 카드가 붙는다
- ⓓ 현행 유지(34.2) + 카드형만 예외로 문서화

### 함께 처리할 것

- `budget_list_page.dart:397` `_buildCurrentWeekHero` 의 `EdgeInsets.all(20)`·`circular(16)`
  **리터럴 2건** 토큰화(같은 화면에서 리듬을 깨는 잔존)
- 가드 확장: `vertical_rhythm_guard_test` 에 **카드형 항목 사이** 단정 추가 →
  우리 타일 · `ListTile` · 카드 **3계열이 같은 승인값**을 지나가게 묶는다
- ratchet/일관성 래칫 재확인(`Card` 내부 리터럴이 줄면 baseline 하향)

### 직전 회차들에서 아직 개별 확인 안 된 항목

- 거래 탭 **±1일 버튼** 라벨 중앙 · 폼 여백 · 자산 탭 편집 모드 320px 3열 ·
  1줄 `ListTile` 34dp 터치감(설정 메뉴)

### 그 다음 대기열 (착수 순서 아님)

1. `ListTile` **87건/39파일** → `EntityTileRow`(폭 계약·어포던스. 세로 리듬은 해결됨)
2. 좌우 비대칭 여백 32건 · 하드코딩 색 474건(래칫 하향)
3. 차트 색 체계 통일(`BbColors.series`)
4. 미기록 200건 초과 달 추가 페이지 로드 UI · 개인 자산(ASSET-PRIVATE)
5. 죽은 화면 정리(`DashboardPage`·`PaymentMethodPage`·`CategoryPage`·`/transfers`)
6. P4 월말 알림 · Android 배포 · 카카오 비즈앱 전환

### 회차 밖 트랙 — 사용자 확인만 필요

- 정산 스냅샷 A1~A10 / B1~B7 / C1~C5 — `docs/sessions/2026-07-27_1_result.md §4.1`
