...
<!-- HNS:STATE -->
- **단계**: 세로 리듬 3차(20dp + 전체 적용) — **배포 완료 · 사용자 라이브 검증 대기**(2026-08-21).
  PR #308 머지 · `main`=`14b13bf` · run 32467170752 `verify-live` success · 회차 **미종결**
- **이번 회차 승인값**: 항목 사이 **20.0dp @390 · 25.0 @960** — **모든 목록**에 동일.
  우리 타일은 `space.xs`(4,4), 프레임워크 `ListTile` 은 테마의
  `minVerticalPadding`(=사이/2) + `minTileHeight`(=사이+제목줄) 두 값. 87곳 개별 수정 없음
- **함께 고친 별건**: 스크롤 TabBar 3곳 `tabAlignment: center`
  (M3 기본 `startOffset` = 왼쪽 52dp → 통계 탭 오른쪽 21dp 잘림)
- **1차 실패 교훈(반드시 유지)**: 사용자 지표를 내 지표로 치환하면 실패한다 —
  행 높이(아바타 67%) ≠ 항목 사이(패딩 57%). 렌더 기하로 재고 체감 %를 사다리로 제시한다.
  요청이 한 축이면 **한 축만** 바꾼다 → `feedback_user_metric_definition`
- **첫 행동**: §3 NEXT 의 라이브 검증 3항목 확인 요청. 0단계는 오프라인 배너 확인
- **가드**: 스윕 37건 · 계단 4종 · 타일 8종 · **세로 리듬 4종(신설)** ·
  리터럴 ratchet 1481 · 일관성 래칫(A0/C0/D0/G0)
- **CI 게이트(6)**: analyze 신규 0 / `flutter test` **1069** / `./gradlew test` /
...
상단 **오프라인 배너 없음** 확인 → 새로고침 1회. `feedback_live_verification_online_precheck`

### 확인 항목 (승인값: 항목 사이 20.0dp @390 · 25.0 @960 — 모든 목록 동일)

1. **거래 탭 목록** — 항목 사이가 좁아졌는가(34.8 → **20.0dp**, 가장 큰 변화 지점)
2. **자산 탭 · 분석>예산 자산 현황 · 카테고리 · 포켓** — 18 → **20** 으로 살짝 넓어졌는가
3. **설정(더보기) 메뉴 · 선택 시트(카테고리/결제수단 고르기)** — 1줄 항목도 좁아졌는가.
   ⚠ 1줄 타일 높이 **34dp** = 44dp 터치 하한 미달 → **누르기 불편하지 않은지** 함께 판정
4. **정산·리포트·반복거래·이체 목록** — 같은 리듬인가
5. **분석>통계 상단 탭** 정렬(2차 fix) — 왼쪽 빈칸 없이 4개가 들어오는가

### 결과별 사전 판정 기준

- "1줄 항목이 누르기 불편" → `kBbBoxSpec[listRowMinHeight]` min 34 → **44**(사이 30dp) 한 값
- "카드형 목록(포켓·지출계획)만 아직 넓다" → `cardTheme.margin` 세로 `sm`→`xs`
  (지금은 카드 분리를 의도해 제외)
- "더/덜 좁게" → 우리 타일은 `kBbSpaceSpec[xs]`, ListTile 은 `listRowPadV`/`listRowMinHeight`.
  **세 값이 같은 사이를 만들도록 가드가 묶여 있다** — 따로 바꾸면 테스트가 깨진다
- "잘 됐다" → **회차 종결**. 같은 세션에서 다음 단계 착수 금지(`feedback_round_boundary_clear`)

### 직전 회차들에서 아직 개별 확인 안 된 항목

- 거래 탭 **±1일 버튼** 라벨 중앙 · 폼·카드 여백 · 자산 탭 편집 모드 320px 3열

### 그 다음 대기열 (착수 순서 아님)

1. `ListTile` **87건/39파일** → `EntityTileRow` 전환(폭 계약·어포던스. **세로 리듬은 해결됨**)
