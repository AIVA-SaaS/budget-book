...
<!-- HNS:STATE -->
- **단계**: **9차 구현 중 — 계측기 완료. S7(소유권 이전) 확인 대기.** 축 = 앱 전역 간격·아이콘·타이포
- **상태**: in_progress · main `410f791` · 8차 라이브 통과 = 계층 관계 회차 종결
- **갱신**: 2026-08-26
- **★[측정] 원인 ⓿ = 블록 간격에 소유자가 없다**: 보이는 간격 = `spacer + 위 margin + 아래 margin`,
  세 항이 서로를 모른다. 같은 `SizedBox(height:16)` 이 **16.00 / 26.00 / 36.00** 으로 보인다
  (비카드↔비카드 / 카드↔비카드 / 카드↔카드 @390). 24→24/34/44 · 32→32/42/52.
  **소스 3종이 렌더 9종**. `BbCardTile` margin 4.00 · 생 `Card` 5.10 · override 33건 · 갈림 파일 42개
- **[측정] 증상 기제**: "붙은 채로" = 16.00 < 항목 사이 **20.00**(역전 −4.00) /
  "통일 안 됨" = 한 화면에 16.00 과 36.00 이 **2.25배** 차로 공존
- **★토큰 추가로는 안 고쳐진다**: `gapV(block)` 24.00 을 카드 사이에 넣으면 보이는 값은 **44.00**.
  → **S7 = 소유권 이전**(카드 세로 margin 0 · 항목 사이 20.0 소유자를 목록 호스트로 · 값 불변)
- **[결정] 사다리 7단 확정**: `block`(24→32) 한 단만 신설, `region` 폐기(사용자 승인).
  앵커 근거 정정 — 리터럴 최빈값 16 은 **서열 위반**이라 후보가 아니고, 서열 만족 최빈이 **24**다
- **[측정] 정본은 이미 있다**: `BbSpace.gapV(BbSpaceToken)` = `bb_scale.dart:356` — 봉인·이관 문제
- **[측정] 게이트**: `ui_pattern` acknowledged(지문 15:2026-08-26) — frontend 편집 허용
- **[측정] 커밋 1 완료**: S7 소유권 이전 + `block`·`cardItemGap` 신설. V3 가 새 기제로 20.0/25.0 그대로 통과
  (중간 상태 12.0/17.0 실패 = 가드가 이전을 강제했다). analyze 14 · test 1,222 전량 · 리터럴 1,461→1,454
- **[측정] 커밋 2 완료**: 세로 간격 전역 이관 — **도달 가능 화면 세로 리터럴 0**. 리터럴 1,461→**968** ·
  래칫 H 31→28. 서열 단조 확인: 필드 `xxl` 15.30 < 항목 20.00 < 블록 24.00 @390
- **⚠ 넓은 폭에서 토큰 > 리터럴**: `xxl` 21.9 vs 16 · `block` 29.2 vs 24 @800 — 데스크톱 여백이 넓어진다(설계된 곡선)
- **[측정] 커밋 3 완료**: 폰트 83 · 아이콘 165 이관 + **서열 단조성 가드 19케이스** 신설.
  리터럴 **1,461 → 719**(−742). analyze **10**(기준선 14) · test **1,241 전량** · build web EXIT=0
- **★대조군이 가드를 반려했다**: 키 위젯 rect 로 재니 대조군이 통과 — `Card` RenderBox 가 margin 을
  포함한다. 지표를 **칠해진 표면**으로 바꿔 스프레드 8.0dp 로 실패 확인. 통과하는 대조군은 축이 틀린 것
- **blocker**: 없음
<!-- /HNS:STATE -->
...
<!-- HNS:NEXT -->
**9차 구현 중(항목 89). 계측기 완료 — S7 확인 대기.**

### 사용자 확인 1건 (되돌리기 비싼 갈림길)

**S7 = 블록 간격의 소유권 이전**을 진행할지. 토큰만 추가하면 카드 사이가 44.00 으로 벌어져
요구가 달성되지 않는다 `[측정]`. S7 은 값(항목 사이 20.0/25.0)을 **보존**하지만 만드는 주체를
카드 margin → 목록 호스트로 옮기고 `Card` 65곳 · `margin:` override 33곳에 닿는다.
대안은 블록 간격을 이웃 종류별로 보정하는 것인데, 호출부가 이웃을 알아야 해서 취약하다.

### 착수 순서

1. **커밋 1 — 폰트·아이콘** (S7 과 무관, 선행 가능. 도메인 ★3 순서상 먼저다)
   `fontSize:` 92 → 0 · 아이콘 `size:` 204 → `BbIconRole`. 리터럴 14(27곳)는 `sm` 하한 15 로 올린다
2. **커밋 2 — S7 소유권 이전 + `block` 토큰 + 세로 간격 5경로 → `gapV` 1경로**(536건)
3. **커밋 3 — 서열 단조성 가드 R1~R5** + PILOT 확장 + baseline + `audit_ui_consistency.py` 6→7.
   **대조군 필수** — 변경 전 lib 에서 16.00 vs 36.00 공존과 −4.00 역전이 재현되어야 한다
4. 로컬 CI 전부(analyze ≤14 · test 전량 · build web · 색 래칫) → PR → 배포 → 라이브 검증

### 완료 기준 (기획서 §8)

도달 가능 화면 세로 리터럴 0 · `fontSize:` 0 · 아이콘 `size:` 0 / **세로 margin 갖는 카드 0개** /
리터럴 baseline 1,461 감소 / 래칫 H 31 감소 / R1~R5 + 대조군 실패 기록 /
스윕 7폭 × 3배율 오버플로 0 / **7차(행 간 높이 0.00) · 8차(정렬 0.00) 무회귀** / 라이브 검증 통과

### 대기열

1. `ListTile` 87건 이관 · 2. 주간 카드 320dp 오버플로우 · 3. 좌우 비대칭 32건 · 하드코딩 색 474
4. `Divider` 57건 정리 · 5. radius 123건/7종 · 6. 대장 본체 104KB 롤오프
7. 미기록 200건 초과 페이지 로드 UI · 개인 자산 · 죽은 화면 6개 · P4 알림 · Android
