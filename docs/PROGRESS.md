...
<!-- HNS:STATE -->
- **단계**: 세로 리듬 3차(20dp + 전체 적용) — **구현·로컬 CI 6종 통과 → 배포 진입**(2026-08-21).
  2차(PR #306)는 사용자 판정 **"잘 되고 마음에 든다"** = 방향 확정
...
상단 **오프라인 배너 없음** 확인(있으면 재연결 후 새로고침). `feedback_live_verification_online_precheck`

### 확인 3항목

1. **항목 1과 2 사이가 좁아졌는가** — 자산 탭(더보기>자산 관리>결제수단) · 분석>예산 맨 아래
   자산 현황 · 카테고리 · 포켓. 승인값은 **사이 18.0dp @390**(이전 28.0)
   - 함께: **테두리·좌우 여백은 이전 그대로**인가(이번엔 안 건드렸다. 바뀌어 보이면 회귀다)
2. **분석 > 통계 상단 탭**(카테고리별·추이·전년 비교·결제수단별) — 왼쪽 죽은 여백이 사라지고
   4개가 화면에 들어오는가. ⚠ 320/360px 폭에서는 **여전히 스크롤**이다(콘텐츠 359dp)
3. **누르기 불편하지 않은가** — 행 높이가 48 → 38dp 로 44dp 터치 하한을 밑돈다(감수한 선택)

### 결과별 사전 판정 기준

- "아직 넓다" → 사다리 다음 칸: **14.0dp**(패딩 `xs` + 아바타 32→28, 아이콘도 조금 작아진다)
- "너무 좁다 / 누르기 불편" → **22.2dp**(`space.xs` → `space.sm` 한 줄 되돌림)
- "통계 탭이 아직 이상하다" → 아이콘 제거로 콘텐츠 359 → 244dp(320px 에서도 전부 들어옴).
  이번엔 요청 범위 밖이라 안 했다
- "잘 됐다" → **회차 종결**. 같은 세션에서 다음 단계 착수 금지(`feedback_round_boundary_clear`)

### 직전 회차에서 아직 개별 확인 안 된 항목

- 거래 탭 **±1일 버튼** 라벨 중앙 · 폼·카드 여백 · 자산 탭 편집 모드 320px 3열

### 그 다음 대기열 (착수 순서 아님)

1. `ListTile` **87건/39파일** → `EntityTileRow` (분석>예산 예산 항목이 아직 프레임워크 높이 48+)
