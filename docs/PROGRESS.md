...
<!-- HNS:STATE -->
- **단계**: 5차(항목 간 간격 전역 통일) **구현 완료 · 로컬 CI 6종 통과 → 배포 대기**
- **상태**: in_progress
- **갱신**: 2026-08-24
- **계약(개정)**: 지표는 **텍스트↔텍스트 20.0 @390 / 25.0 @960**.
  `사이 = 행 박스 − 행 잉크` · `박스 = max(아바타, 액션 슬롯 44, 텍스트) + 2×여백`
  → **여백이 아니라 슬롯이 리듬을 정한다**
- **행 계열별 값** `[측정 2026-08-24]`: 액션 없는 행 **20.0/25.0** · 액션(44) 든 행
  **24.0**(터치 하한을 지킨 최소) · 주간 카드 내부 행 **20.0/25.0** · 예산 최상위 **20.0/25.0**
...
- **가드**: 세로 리듬 V1/V1-b/V3 · **슬롯 리듬 4종(탭 라우팅 히트 영역 포함)** ·
  채택 가드 · 계측기 3종 · 리터럴 래칫 **1461**
- **다음**: 커밋 → PR → 배포 → 라이브 검증(§3 NEXT)
- **blocker**: 없음
<!-- /HNS:STATE -->
