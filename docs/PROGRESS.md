...
<!-- HNS:STATE -->
- **단계**: UI 크기 동적 체계 회차 **종결(라이브 검증 통과, 2026-08-20)**.
  PR #299+#300 · `main`=`f424143` · 정본 `docs/sessions/2026-08-19_1_typography-anchor_result.md`
- **다음 회차**: **`BbDensity` 흡수 + 여백·간격 조정** — 지시 접수, **기획 전·코드 0줄**.
  요청과 착수 실측은 §3 NEXT (그대로 이어서 진행)
- **첫 행동**: §3 NEXT 읽고 **기획부터**. 승인 게이트는 기획 단계뿐.
  `ui_pattern` STRUCTURAL_FIX_REQUIRED → `acknowledge-gate.sh frontend <새 기획서>` 가 편집 전제
- **규칙 정본**: `~/.claude/domains/12-ui-scaling.md`(전 앱 기본값, 자동 라우팅).
  clamp 는 비율 아닌 **가독 px** · 판정은 **컨테이너 폭** · 이관은 **폰트→여백→밀도** 순 ·
  L4 고정 px 는 `0/1/0.5/44` 만
- **기반(새로 만들지 말 것)**: `bb_scale.dart`(`BbType` 폰트6·아이콘3·크롬높이 / `BbSpace` /
  `BbScaleScope` 유효폭 단일소스) · `app_theme.responsive()`(15슬롯+밀도+7컴포넌트) ·
  `bb_tab.dart` · `BbColors` · `EntityTileRow` · `BbDensity`
- ⚠ **타이포 기준점 = 자산 탭**(사용자 검증). 가드가 양 끝 정확 일치를 고정 — **함부로 바꾸지 말 것**
- **가드**: 타일 8종 + 스윕 **32건**(오버플로0·역행금지·크롬70%래칫·44dp·기준점정합·본문반응)
  + 리터럴 ratchet(baseline **1699**, 시범 3파일 0). 하드코딩 색·`ListTile`·`MediaQuery width`·
  크기 리터럴은 **테스트가 막는다**
- ⚠ CI Flutter 최신(로컬 3.41.2 vs CI 3.47.0) → 실패 시 `gh run view <id> --log-failed` 부터
- ⚠ **배포 성공 판정은 산출물 신선도**(`last-modified`+`content-length`), 워크플로 상태 아님.
  `gh run watch` 를 파이프 뒤에서 `$?` 로 읽지 마라
- **CI 게이트(5)**: analyze 신규 0 / `flutter test` **1060** / `./gradlew test` /
  `build web --release` / `tool/check_ui_scaling.py` + 배포 후 `verify-cache-headers.sh`
...
<!-- HNS:NEXT -->
**직전 회차 종결. 아래로 새 회차 시작 — 기획부터.**

### 요청 (사용자, 2026-08-19 — 원문)

> 잘 됐다. 별도 회차에서 진행될 때 여백, 간격 같은 부분도 확인되어 적절하게 조정될 수 있게 해줘

**범위 2건**: ①**`BbDensity` 흡수 = 경쟁 경로 0개**(폰트·여백이 아직 3단 계단으로 별도 정의.
직전 회차에서 이것 때문에 "자산만 반응, 나머지 붙박이" 가 났고 **폰트·여백 두 번 겪었다**)
②**여백·간격 적정화**(사용자 지시)

### 착수 실측 — 재조사 불필요 `[측정 2026-08-20]`

여백은 폰트와 **정확히 같은 구도**다:

```
            320   360   390   768   960  1440  2560
BbSpace md  7.7   7.7   7.7   8.1   8.3   8.3   8.3   ← 7% 범위
padH         10    10    10    14    16    16    16   ← 60% 반응
padV          8     8     8    10    12    12    12
gap           6     6     6     8    10    10    10
avatar       32    32    32    38    40    40    40
```

- `BbSpace.factor` **0.964~1.035** — 폭 8배에 여백 **0.6dp** = 사실상 상수
- **원인**: `factor = sqrt(body(W)/14)` 인데 body 가 `13~15` 로 좁게 clamp 돼 비율이
  `13/14~15/14` 뿐. **층 의존의 부작용** — 폰트 하한/상한을 좁히면 여백도 묶인다
- 배율 결합은 **정상**(390px, 1.0→1.6 에서 factor 0.964→1.219 / md 7.7→9.8)
- **기준점은 또 `BbDensity`**(padH 10/14/16) — 폰트 때와 같이 자산 탭이 검증된 정답

### 착수 유의

- 폰트 회차 해법을 재사용: `BbDensity` 3단 값을 통과하도록 역산. 단 여백은 body 비율에
  묶여 있어 **결합식 자체를 손봐야** 한다 — 후보 ⓐ`factor` 를 폭에서 직접 계산
  ⓑ`kBbSpaceReferenceBody` 재설정 ⓒ지수(현 0.5) 조정. **기획에서 판정**
- **위험**: 자산 탭은 승인된 표면. 768px 대에서 계단↔곡선 차이가 생긴다(폰트는 15 vs 15.2).
  허용 오차를 기획에서 정하고 **가드로 고정**
- 체계 밖 리터럴 `EdgeInsets` **446** · `SizedBox(w/h)` **747**. 전수는 이 회차 아님 — ratchet 이 낮춘다
- ⚠ 스윕의 "여백은 폰트를 따라가되 제곱근" 테스트가 결합식을 고정한다 → 바꾸면
  **의도적으로 갱신**할 것(무심코 지우지 말 것)
- 그 다음 대기열·회차 밖 트랙은 **§3 아래 절**
<!-- /HNS:NEXT -->
