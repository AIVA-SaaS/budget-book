...
<!-- HNS:STATE -->
- **단계**: 세로 리듬 2차 — **승인·구현·로컬 CI 6종 통과 → 커밋·PR·배포 진입**(2026-08-21).
  1차(PR #303)는 되돌림 완료(PR #305, 라이브 확인: 번들 크기 5515085 = 이전과 동일)
- **사용자 판정**: ① 테두리·좌우 여백은 **이전이 나았다**(요청 범위 밖 변경 = 내 오류)
  ② 위아래 간격은 **동일하거나 넓다** — padV 1.6dp 축소로는 체감이 안 온다
  ③ **분석>통계 상단 탭(카테고리별·추이·전년 비교) 정렬이 이상해졌다** — 원인 미확인
- **측정 완료** `[측정]`: 행 높이 48.0dp @390 = **아바타 32(67%)** + padV 16(33%),
  행 사이 gap 0. 1차 변경은 −6.8% 였다 → 지배 변수는 padding 이 아니다
- **통계 탭 원인 확정** `[측정]`: M3 스크롤 탭 기본 `tabAlignment=startOffset`(왼쪽 52dp) +
  #299 로 넓어진 탭 폭 → 오른쪽 21dp 잘림. `TabAlignment.center` 로 해소(3곳 공통)
- **첫 행동**: 재기획서 승인 여부 확인 →
  `docs/sessions/2026-08-21_2_vertical-rhythm-remeasured_plan.md`
- **불변식**: 요청은 "**위아래만** 가깝게". 테두리·좌우 여백·구조는 **건드리지 않는다**
- **규칙 정본**: `~/.claude/domains/12-ui-scaling.md` · 승인값 표는
  `responsive_sweep_test.dart` 상단 `anchors` 가 소유
- **가드**: 스윕 37건 · 계단 금지 4종 · 타일 8종 · 리터럴 ratchet **1481**(원상) ·
  일관성 래칫(A0/C0/D0/G0, B32·E87·F474 동결)
