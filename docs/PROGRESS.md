...
<!-- HNS:STATE -->
- **단계**: **9차 배포 완료 — 라이브 검증 대기.** 축 = 앱 전역 간격·아이콘·타이포 체계
- **상태**: verifying · main `6422660` · 번들 2026-08-26 21:44:32 GMT / 5,535,029B(+18,032)
- **갱신**: 2026-08-27
- **★[측정] 원인**: 블록 사이 간격에 **소유자가 없었다** — 보이는 간격 = `spacer + 위 margin +
  아래 margin` 의 합이라 같은 `SizedBox(height:16)` 이 **16.00 / 26.00 / 36.00** 으로 갈렸다.
  게다가 항목 사이 20.0 봉인 대비 최빈 블록 간격이 16 → **서열 역전 4.0dp** = "붙은 채로"
- **[처방] 축마다 소유자 하나, 경계는 위젯 경계**: `block`(24→32)·`cardItemGap`(8.0 상수) 신설(사다리 7단) ·
  카드 세로 margin 0 · 호스트 소유자 3종(`BbCardGap`/`bbCardItems`/`BbCardItem`) ·
  세로 경로 5개 → `gapV` 1개(457건 이관, 도달 가능 화면 잔존 **0**) · 폰트 83 · 아이콘 165
- **[측정] 확정 서열 @390**: 필드 `xxl` 15.30 < 항목 20.00 < 블록 24.00 — 단조
- **★신규 가드 19케이스**(`block_rhythm_guard_test`) — 4회차 만에 처음 **서열**을 잰다.
  **대조군이 이 가드를 1회 반려**했다(키 위젯 rect 로 재니 통과 — `Card` RenderBox 가 margin 포함).
  지표를 **칠해진 표면**으로 바꿔 스프레드 8.0dp 실패 확인
- **[측정] 로컬 CI**: analyze **10**(기준선 14) / test **1,241 전량** / build web EXIT=0 /
  리터럴 **1,461 → 719**(−742, 51%) / 래칫 H 31→28
- **blocker**: 없음
<!-- /HNS:STATE -->
...
<!-- HNS:NEXT -->
**9차 배포 완료(항목 93). 라이브 검증 회신 대기.**

### 라이브 검증 (배포 후)

1. **콘텐츠 블록 사이 간격이 화면 전체에서 일정한가** — 카드끼리든 카드↔비카드든 같아 보인다.
   특히 **분석>예산 · 분석>통계 · 자산 · 더보기** 네 탭
2. **블록 경계가 목록 항목 사이보다 넓은가** — 블록이 "붙어" 보이지 않는다(이번 회차의 핵심)
3. **항목 높이·정렬은 7·8차 그대로다**(예산 카드 묶임 · 접기/펼치기 · 아바타 정렬) — 깨지면 즉시 보고
4. **아이콘·글자 크기가 자산 탭과 어긋나지 않는가** — 자산 탭이 기준점이다
5. **폼·시트**(거래 입력 · 카테고리 · 포켓 · 결제수단)의 필드 사이가 일정한가

### 알려진 귀결 (의도된 변경 — 어색하면 알려주세요)

- **데스크톱(768dp+)에서 여백이 넓어진다** — 토큰이 리터럴보다 크다(`xxl` 21.9 vs 16 @800). 설계된 곡선
- **큰 여백 32 가 24 로 8dp 축소**된다(커플 10곳 · 온보딩 5곳 · 앱정보 3곳 등) — `region` 미신설 결정의 귀결
- 로그인 히어로 여백(60·40)은 그대로다(`ui-fixed`)

### 그 다음 대기열

1. `ListTile` 86건 이관 · 2. 가로 `EdgeInsets`·radius 이관(리터럴 719 잔존) · 3. 좌우 비대칭 32건
4. 하드코딩 색 470 래칫 · 5. `Divider` 57건 정리 · 6. **대장 본체 117KB 롤오프**(경고 발생 중)
7. 죽은 화면 6개 정리 · 미기록 200건 초과 로드 UI · 개인 자산 · P4 알림 · Android
