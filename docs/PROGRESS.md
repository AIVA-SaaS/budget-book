...
<!-- HNS:STATE -->
- **단계**: 카드형 회차 **착수 — 대조군 실측 완료 · 구현 진행 중**(2026-08-24). 회차 미종결
- **상태**: verifying (카드형 회차 착수 — 라이브 검증 대기 항목 있음)
- **승인값(불변)**: 항목 사이 **20.0dp @390 · 25.0 @960**. 3계열이 같은 값을 지나가야 한다:
  우리 타일 `kBbSpaceSpec[xs]` · `ListTile` `listRowPadV`(10,12.5)+`listRowMinHeight`(34,41) ·
  **카드 = `margin xs` + 신규 `cardRowPadV`(6, 8.5)** ← 이번 회차
- **실측 미적용 표면 5개** `[측정 2026-08-24]`: 예산>주간카드 75.2/80.5 ·
  통계>카테고리별 34.2/48.0 · 결제수단별 32.0/32.0 · 전년비교 62.7/67.5 · 요약탭 48.8/76.0
- **이미 적용 확인**: 예산 **항목**(`ListTile`) = 20.0/25.0 — 사용자가 "예산 미적용"으로
  본 것은 항목이 아니라 **주간 카드 목록**이다
- **첫 행동**: 기획서 `docs/sessions/2026-08-24_1_card-rhythm_plan.md` §5(구조적 수정) 대로
  `BbCardTile` 도입 → 호스트 이관 → 가드 A~F → 계측기 재실행으로 20.0/25.0 확인
- **복합 카드 주의**: 전년비교·주간카드는 콘텐츠 자체가 꼬리 여백을 갖는다 → 래퍼 교체만으론
  승인값에 못 닿는다. 내부 여백까지 손대고 **계측기로 재확인**(추정 종결 금지)
- **1·2차 실패 교훈(유지)**: 사용자 지표를 내 지표로 치환하지 마라 · 요청이 한 축이면 한 축만
  → `feedback_user_metric_definition`
- **가드**: 스윕 37건 · 계단 4종 · 타일 8종 · 세로 리듬 9종(+카드 V3 예정) ·
  리터럴 ratchet 1481 · 일관성 래칫(A0/C0/D0/G0)
- **CI 게이트(6)**: analyze 신규 0 / `flutter test` **1074** / `./gradlew test` /
  `build web --release` / `check_ui_scaling.py` / `audit_ui_consistency.py`
- **blocker**: 없음 · **갱신**: 2026-08-24
- **갱신**: 2026-08-24
<!-- /HNS:STATE -->
...
<!-- HNS:NEXT -->
**카드형 세로 리듬 배포 → 라이브 검증.** 구현·로컬 CI 6종은 통과했다(대장 항목 75).

### 지금 할 일

1. 커밋 → PR → 머지 → NAS 배포(개인 계정 자동 진행 범위)
2. 배포 성공 판정은 **산출물 신선도**로 한다(`last-modified`/`content-length`) —
   워크플로 상태로 판정하지 않는다 → `feedback_deploy_success_by_artifact`
3. 사용자 라이브 검증 요청. **0단계 = 오프라인 배너 없음 확인**
   (`feedback_live_verification_online_precheck`)

### 사용자가 볼 화면과 기대값 (텍스트↔텍스트, 모바일 390 기준)

- 분석 > 통계 > **카테고리별** 34.2 → **20.0dp**
- 분석 > 통계 > **결제수단별** 32.0 → **20.0dp**
- 분석 > 통계 > **전년 비교** 62.7 → **28.5dp**(초과분은 카드 안 아이콘 블록)
- 분석 > 예산 > **주간** 카드 75.2 → **41.0dp**(초과분은 카드 안 진행바)
- 분석 > 예산 > **월간 항목**: **변화 없음**(이미 승인값) — 사용자가 "예산 미적용"으로
  본 대상은 항목이 아니라 주간 카드였다. 이 점을 함께 확인받는다
- 카드 **테두리 사이는 전 표면 8.0dp 균일**

### 라이브 검증에서 물어볼 것

1. 카테고리별·결제수단별 사이가 거래 목록과 **같은 느낌**인가
2. 카드 **내부**가 답답하지 않은가(@390 세로 여백 8.0 → 6.0) — `[무가드]` 축
3. 전년비교·주간카드가 여전히 넓게 느껴지면 다음 회차에서 **카드 안쪽**(아이콘 블록 36dp ·
   진행바)을 줄인다

### 다음 회차 대기열 (착수 순서 아님)

1. 비목록 카드(요약·히어로·설정) 리듬 + `_buildCurrentWeekHero` 리터럴 2건
2. `ListTile` 87건 → `EntityTileRow`(폭 계약·어포던스)
3. 좌우 비대칭 여백 32건 · 하드코딩 색(래칫 하향) · 차트 색 체계 통일
4. 미기록 200건 초과 달 추가 페이지 로드 UI · 개인 자산(ASSET-PRIVATE)
5. 죽은 화면 정리 · P4 월말 알림 · Android 배포

### 회차 밖 트랙 — 사용자 확인만 필요

- 정산 스냅샷 A1~A10 / B1~B7 / C1~C5 — `docs/sessions/2026-07-27_1_result.md §4.1`
- KI-006 배포 후 확인 · 거래 탭 ±1일 버튼 라벨 중앙 · 폼 여백 · 자산 탭 편집 모드 320px
<!-- /HNS:NEXT -->
