...
<!-- HNS:STATE -->
- **단계**: 5차(항목 간 간격 전역 통일) **배포 완료 → 사용자 라이브 검증 대기**
- **상태**: verifying
- **갱신**: 2026-08-24
- **main**: `dd4aa37`(PR #313) · 배포 판정 근거 `[측정]`: content-length
  5,515,435 → **5,515,901** · last-modified 08-23 23:35 → **08-24 01:07** · verify-live success
- **계약(개정)**: 지표는 **텍스트↔텍스트 20.0 @390 / 25.0 @960**.
  `사이 = 행 박스 − 행 잉크` · `박스 = max(아바타, 액션 슬롯 44, 텍스트) + 2×여백`
  → **여백이 아니라 슬롯이 리듬을 정한다**
- **행 계열별 값** `[측정]`: 액션 없는 행 **20.0/25.0** · 주간 카드 내부 행 **20.0/25.0** ·
  예산 최상위 **20.0/25.0** · 액션(44) 든 행 **24.0**(터치 하한을 지킨 최소 = 유일한 미달)
- **하지 않은 것**: `ListTile` 45파일 아바타 축소 — 승인 표면(거래 목록)의 잉크 사이가
  13.0 이라 잉크 기준 통일은 그 표면에서 멀어진다
- **가드**: 세로 리듬 V1/V1-b/V3 · 슬롯 리듬 4종(**탭 라우팅 히트 영역** 포함) ·
  채택 가드 · 계측기 3종 · 리터럴 래칫 **1461** · `flutter test` **1133**
- **blocker**: 없음
<!-- /HNS:STATE -->
...
<!-- HNS:NEXT -->
**5차 배포 완료 → 라이브 검증.** 0단계 = 오프라인 배너 없음 확인 + 새로고침 1회.

### 확인 3점 (모바일 390 기준)

1. 분석 > 예산 > **주간** — 카드 안 카테고리 행 사이 **28 → 20dp**(이번 회차 최대 변화)
2. 분석 > 예산 하단 **자산 현황** — 그룹 경계 **91 → 69dp**(여백분 20) · 그룹 내 항목
3. **잔액 수정(튠)·⋮ 버튼이 있는 행** — **24.0dp**(20 아님). 터치 하한 44 를 지킨 최소값

### 변화가 없어야 정상인 곳

거래 목록 · 분석>예산 **최상위 항목** · 통계 카드 3표면 — 이번에 건드리지 않았다.

### 판정이 갈리면 (사전 선언)

- 액션 행이 여전히 넓다 → (i) 터치 하한 44 → 40 하향(도메인 12 L4 위반, 승인 필요)
  또는 (ii) 상시 노출 액션을 메뉴로 접기(어포던스 변경, 승인 필요)
- 자산 현황 칩 행이 좁다(16.0) → 칩 행만 여백 xs 복원(사이 24)
- 설정 메뉴·선택 시트가 거래 목록보다 붙어 보인다 → `ListTile` 아바타 축소 재검토
  (잉크 5.0 vs 거래 목록 13.0 — 이때는 승인값 자체를 잉크 기준으로 재협의)

### ⚠ 전역 20 의 전제 — 계측으로 확인된 구조적 제약 `[측정 2026-08-24]`

프레임워크 `ListTile` 은 행 높이를 **텍스트 + `minVerticalPadding`** 으로 정하고 선행
위젯을 그 안에 **중앙 정렬**한다. 그래서 선행 상자를 키우는 보정은 **무시된다**(가설 반증:
아바타 40 + 보정 → 사이 5.0 그대로). 아이콘을 줄이지 않고 전역 20 을 만들려면
**`ListTile` → `EntityTileRow` 이관(87건/39파일)** 이 전제다. 현재 편차:
1줄 20.0 ✅ · 2줄 14.0 · 액션 행 24.0 · 칩 행 16.0.

### 다음 회차 대기열

1. **`ListTile` → `EntityTileRow` 이관 + 행 리듬 단일 소유자(`BbRow`)** — 위 제약의 해소책.
   회귀 위험: 이미 승인된 표면 전부(거래 목록·자산 탭) → 기획·승인 필요
2. **주간 카드 320dp 가로 오버플로우**(6.2~31px, 기존 결함 확인됨)
2. 비목록 카드(요약·히어로·설정) 리듬 + `_buildCurrentWeekHero` 리터럴 2건
3. `ListTile` 87건 → `EntityTileRow`(폭 계약·어포던스)
4. 좌우 비대칭 여백 32건 · 하드코딩 색 래칫 하향 · 차트 색 체계 통일
5. 미기록 200건 초과 달 추가 페이지 로드 UI · 개인 자산(ASSET-PRIVATE)
6. 죽은 화면 정리 · P4 월말 알림 · Android 배포
7. **대장 롤오프** — 본체 136KB(150KB 부터 차단)

### 회차 밖 트랙 — 사용자 확인만 필요

- 정산 스냅샷 A1~A10 / B1~B7 / C1~C5 — `docs/sessions/2026-07-27_1_result.md §4.1`
- KI-006 · 거래 탭 ±1일 버튼 라벨 중앙 · 폼 여백 · 자산 탭 편집 모드 320px
<!-- /HNS:NEXT -->
