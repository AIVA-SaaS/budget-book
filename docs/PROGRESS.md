...
<!-- HNS:STATE -->
- **단계**: 6차 **구현 완료 · 로컬 CI 6종 통과 · 커밋/배포 진입**
- **상태**: in_progress
- **갱신**: 2026-08-24
- **main**: `de971e2` (5차 배포분 `dd4aa37` 라이브 반영됨)
- **처방(구현됨)**: 행 리듬 단일 소유자 — 슬롯을 **세로 흐름 밖**으로 빼서
  `박스 = max(잉크 + 2 × 여백, 슬롯 44)`, 여백은 조건 없이 **대칭**(`xs`)
- **신고 ③ 결과** `[측정]`: 자산 탭·카테고리 탭·자산 현황 전부 위 = 아래
  (칩 행 4.0/4.0 · 1줄+슬롯 행 6.0/6.0). 액션 유무로 갈렸던 사이 16.0 vs 18.0 →
  **18.0 단일**, 1줄+슬롯 행 24.0 유지(터치 하한)
- **신고 ① 결과** `[측정]`: 거래 타일 선행 캡션이 **부제목과 중복**이라 삭제 →
  선행 50 → 32, 겹침 −6.0 → 0(오버플로우 예외 소멸). `minTileHeight` 는 안 건드렸다
- **신고 ② 결과** `[측정]`: 예산 부제목의 `/ 400,000원` 이 trailing 금액과 중복이라
  삭제 + `OneLineLabel` → 행 잉크 93 → **43.0**, 사이 20.0(승인값 그대로)
- **가드**: `row_balance_guard_test` 20건 신설(**대조군에서 5건 실패** 확인) ·
  주석 문구로 오통과하던 소스 봉인 2건을 주석 제거 스캔으로 교체
- **미해결**: 자산 현황 **그룹 경계** 67.0dp `[측정]` — 행 축이 아니라 그룹 축
  (그룹 박스 20 + 헤더 + 44 슬롯 슬랙의 합). 이번 회차 범위 밖
- **blocker**: 없음
<!-- /HNS:STATE -->
