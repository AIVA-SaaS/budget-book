...
<!-- HNS:STATE -->
- **단계**: 6차 **진단 완료 · 구현 대기**(측정 모드). 라이브 회신 3건의 원인 전부 특정
- **상태**: planning
- **갱신**: 2026-08-24
- **main**: `c4541c3` (5차 배포분 `dd4aa37` 라이브 반영됨)
- ⚠ **내가 만든 회귀 2건이 라이브에 있다** `[측정]`:
  - 3차(PR #308) `minTileHeight: 34` → 거래 타일 선행 Column(50)이 박스(45)를 넘어
    **행 사이 −6.0 = 겹침**(사용자 신고 ①)
  - 5차(PR #313) `hasTallSlot ? 0 : space.xs` → 액션 행 위 12.0 / 아래 **4.0**
    (사용자 신고 ③ — 자산 탭·카테고리 탭·자산 현황)
- **신고 ②는 회귀 아님**: 예산 행 위/아래 10.0/10.0 대칭이고, 높이 93dp 의 48%는
  부제목 **3줄 랩**(폭 145.6dp). 축이 여백이 아니라 **폭**이다
- **처방(승인 대기)**: 행 리듬 단일 소유자 — `박스 = max(잉크, 선행, 슬롯 44) + 2×padV`,
  **위 = 아래**, 사이 = 2×padV(1줄+44슬롯 행만 24.0)
- **기획서**: `docs/sessions/2026-08-24_3_row-balance_plan.md`(전수 계측·가드 매핑·미해결)
- **계측기 4종 상주**: `measure_row_balance` · `measure_global_gap` · `measure_card_gap` ·
  `measure_budget_gap`
- **blocker**: 없음(승인 후 착수)
<!-- /HNS:STATE -->
...
<!-- HNS:NEXT -->
**6차: 행 안·행 사이 위아래 균형 — 기획서대로 구현.** 기획서
`docs/sessions/2026-08-24_3_row-balance_plan.md` 를 먼저 읽어라. 재계측 불필요.

### 착수 순서 (회귀 위험 순)

1. `entity_tile_row.dart` — `hasTallSlot ? 0 : space.xs` 를 **대칭 규칙**으로 교체.
   `박스 = max(잉크, 선행, 슬롯 44) + 2 × listRowPadV`, 위 = 아래.
   → 신고 ③(자산 탭·카테고리 탭·자산 현황) 해소
2. 신고 ① — **택1 필요**(기획서 §5-1): (a) 거래 타일의 결제수단 글자를 `subtitle` 로
   이관해 선행 높이를 32 로 낮춘다 (b) `minTileHeight` 를 선행 최대 높이(50)+여백으로 올린다.
   (a)가 구조적으로 맞지만 거래 목록 시각이 바뀐다
3. `account_balance_card`(자산 현황) ≡ 자산 탭 **같은 위젯 계약**으로 통일(사용자 지시)
4. 신고 ② — **폭 축, 별건**: 예산 부제목 3줄 랩. `maxLines: 1`+문자열 축약 vs
   trailing 금액을 부제목으로 흡수 → 사용자 판단 필요(기획서 §5-2)

### 가드 (기획서 §4)

- 신규 `row_balance_guard_test`: **위 − 아래 ≤ 0.51**(대칭) + **선행이 박스를 넘지 않음**
  (음수 = 겹침이면 실패) + 표면 목록을 코드가 열거
- 기존 `row_slot_rhythm_guard_test` 확장(사이 20.0/25.0, 1줄+슬롯 24.0) · 탭 라우팅 44 유지

### 검증

계측기 4종 재실행 → 모든 표면에서 `위 == 아래`, 선행 넘침 0, 사이 20.0/25.0 확인.
그다음 CI 6종 → PR → 배포 → 라이브 3점(거래 아이콘 아래 / 자산·카테고리 항목 / 예산 높이).

### 그 다음 대기열

1. `ListTile` 87건/39파일 → `EntityTileRow` 이관(프레임워크가 높이를 소유하는 구조 해소)
2. 주간 카드 320dp 가로 오버플로우 · 비목록 카드 리듬 · `_buildCurrentWeekHero` 리터럴 2건
3. 좌우 비대칭 여백 32건 · 하드코딩 색 래칫 · 차트 색 체계
4. 미기록 200건 초과 페이지 로드 UI · 개인 자산 · 죽은 화면 정리 · P4 알림 · Android
5. **대장 롤오프** — 본체 137KB(150KB 부터 차단)
